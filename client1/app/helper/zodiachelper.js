function getZodiacSign(day, month) {
  ...
  let index = zodiacDates.findIndex(sign => month === sign.month && day <= sign.day);
  if (index === -1) {
    index = 0;
  }
...
export function getHoroscope(birthday) {
  const [year, month, day] = birthday.split('-').map(Number);
  const zodiacSign = getZodiacSign(day, month);
  const chineseZodiac = getChineseZodiac(birthday)

  return { horoscope: zodiacSign, zodiac: chineseZodiac }
}
...
function getChineseZodiac(birthDate) {
  const zodiacDates = [
    ...
    { start: '1918-02-09', end: '1919-01-28', sign: 'Horse' },
    { start: '1917-01-22', end: '1918-02-08', sign: 'Snake' },
    { start: '1916-02-04', end: '1917-01-21', sign: 'Dragon' },
    { start: '1915-02-15', end: '1916-02-03', sign: 'Rabbit' },
    { start: '1914-01-26', end: '1915-02-14', sign: 'Tiger' },
    { start: '1913-02-05', end: '1914-01-25', sign: 'Ox' },
    { start: '1912-02-15', end: '1913-02-04', sign: 'Rat' }
  ];

  const inputDate = new Date(birthDate);

  for (const zodiac of zodiacDates) {
    const startDate = new Date(zodiac.start);
    const endDate = new Date(zodiac.end);
  ...
  return 'Zodiac sign not found for the given date';
}